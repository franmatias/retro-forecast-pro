...
import { Chart, CategoryScale, LinearScale, PointElement, LineElement, BarElement, Title, Tooltip, Legend, BarController, Filler } from 'chart.js'
...
const rainfallChart = ref<HTMLCanvasElement | null>(null)
...
function getTotalRainfall(): string {
...
function getMaxRainfallDay(): string {
  if (!props.historicalData.length) return 'N/A'
  ...
  props.historicalData.forEach(data => {
    if (typeof data.rainfall === 'number' && data.rainfall > maxRainfall) {
      maxRainfall = data.rainfall
      maxDay = data.date
    }
  })
  
  if (maxRainfall <= 0) return 'Sin lluvia'
  ...
  return `${formattedDate} (${maxRainfall.toFixed(1)}mm)`
}

function getAverageRainfall(): string {
  if (!props.historicalData.length) return 'N/A'
...
function processWeeklyView(): { labels: string[], datasets: ChartDataset[] } {
  ...
  props.dates.forEach(dateStr => {
    ...
    props.selectedLocations.forEach(locationId => {
      ...
      if (dataForDate && typeof dataForDate.rainfall === 'number') {
        if (weeklyData.get(weekLabel)?.[locationId]) {
          weeklyData.get(weekLabel)![locationId].rainfalls.push(dataForDate.rainfall);
          weeklyData.get(weekLabel)![locationId].count++;
          
          // Sumar la precipitación total de la semana
          const total = weeklyData.get(weekLabel)![locationId].totalRainfall || 0;
          weeklyData.get(weekLabel)![locationId].totalRainfall = total + dataForDate.rainfall;
          
          // Actualizar máxima precipitación
          if (!weeklyData.get(weekLabel)![locationId].max || 
              dataForDate.rainfall > weeklyData.get(weekLabel)![locationId].max!) {
            weeklyData.get(weekLabel)![locationId].max = dataForDate.rainfall;
          }
        }
      }
    });
  ...
  // Crear datasets para cada ubicación (total semanal de precipitaciones)
  const datasets = props.selectedLocations.map(locationId => {
    const displayName = props.getLocationDisplayName(locationId);
    const color = getRandomColor();
    const data = weekLabels.map(week => {
      ...
      if (weekData && weekData.count > 0) {
        // Mostrar suma total de precipitación semanal
        return weekData.totalRainfall || 0;
      }
      return null;
    });
...
function processMonthlyView(): { labels: string[], datasets: ChartDataset[] } {
  ...
  props.dates.forEach(dateStr => {
    ...
    props.selectedLocations.forEach(locationId => {
      ...
      if (dataForDate && typeof dataForDate.rainfall === 'number') {
        if (monthlyData.get(monthLabel)?.[locationId]) {
          monthlyData.get(monthLabel)![locationId].rainfalls.push(dataForDate.rainfall);
          monthlyData.get(monthLabel)![locationId].count++;
          
          // Sumar la precipitación total del mes
          const total = monthlyData.get(monthLabel)![locationId].totalRainfall || 0;
          monthlyData.get(monthLabel)![locationId].totalRainfall = total + dataForDate.rainfall;
          
          // Actualizar máxima precipitación
          if (!monthlyData.get(monthLabel)![locationId].max || 
              dataForDate.rainfall > monthlyData.get(monthLabel)![locationId].max!) {
            monthlyData.get(monthLabel)![locationId].max = dataForDate.rainfall;
          }
        }
      }
    });
  ...
  // Crear datasets para cada ubicación (total mensual de precipitaciones)
  const datasets = props.selectedLocations.map(locationId => {
    const displayName = props.getLocationDisplayName(locationId);
    const color = getRandomColor();
    const data = monthLabels.map(month => {
      ...
      if (monthData && monthData.count > 0) {
        // Mostrar suma total de precipitación mensual
        return monthData.totalRainfall || 0;
      }
      return null;
    });
...
function getDailyTableData(): Array<Record<string, string | number>> {
  ...
  if (viewMode.value === 'days' && hourlyData.value.length > 0) {
    ...
    return timeLabels.map(time => {
      ...
      props.selectedLocations.forEach(locationId => {
        ...
        const displayName = props.getLocationDisplayName(locationId);
        if (hourItem) {
          rowData[displayName] = `${hourItem.rainfall.toFixed(1)} mm`;
        } else {
          rowData[displayName] = '0.0 mm';
        }
      });
      return rowData;
    });
  ...
  return props.dates.map(date => {
    ...
    props.selectedLocations.forEach(locationId => {
      ...
      if (data) {
        const displayName = props.getLocationDisplayName(locationId);
        rowData[displayName] = typeof data.rainfall === 'number' ? 
...
function getWeeklyTableData(): WeekRow[] {
  ...
  props.dates.forEach(dateStr => {
    ...
    props.selectedLocations.forEach(locationId => {
      ...
      const dataForDate = props.historicalData.find(d => d.date === dateStr && d.locationId === locationId);
      if (dataForDate && typeof dataForDate.rainfall === 'number' && weeklyData.get(weekLabel)) {
        const weekDataLoc = weeklyData.get(weekLabel)![locationId];
        if (weekDataLoc) {
          weekDataLoc.rainfalls.push(dataForDate.rainfall);
          weekDataLoc.count++;
          
          // Actualizar total y máximo
          weekDataLoc.totalRainfall = (weekDataLoc.totalRainfall || 0) + dataForDate.rainfall;
          if (dataForDate.rainfall > (weekDataLoc.max || 0)) {
            weekDataLoc.max = dataForDate.rainfall;
          }
        }
      }
    });
  ...
  weekRows.forEach(row => {
    props.selectedLocations.forEach(locationId => {
      if (weeklyData.get(row.date)) {
        ...
        if (weekData && weekData.count > 0) {
          const total = weekData.totalRainfall || 0;
          // Eliminar la variable avg no utilizada
          const max = weekData.max || 0;
          
          row[displayName] = `${total.toFixed(1)} mm (Máx: ${max.toFixed(1)})`;
        }
      }
    });
  });
...
function getMonthlyTableData(): MonthRow[] {
  ...
  props.dates.forEach(dateStr => {
    ...
    props.selectedLocations.forEach(locationId => {
      ...
      const dataForDate = props.historicalData.find(d => d.date === dateStr && d.locationId === locationId);
      if (dataForDate && typeof dataForDate.rainfall === 'number' && monthlyData.get(monthLabel)) {
        const monthDataLoc = monthlyData.get(monthLabel)![locationId];
        if (monthDataLoc) {
          monthDataLoc.rainfalls.push(dataForDate.rainfall);
          monthDataLoc.count++;
          
          // Actualizar total y máximo
          monthDataLoc.totalRainfall = (monthDataLoc.totalRainfall || 0) + dataForDate.rainfall;
          if (dataForDate.rainfall > (monthDataLoc.max || 0)) {
            monthDataLoc.max = dataForDate.rainfall;
          }
        }
      }
    });
  ...
  monthRows.forEach(row => {
    props.selectedLocations.forEach(locationId => {
      if (monthlyData.get(row.date)) {
        ...
        if (monthData && monthData.count > 0) {
          const total = monthData.totalRainfall || 0;
          // Eliminar la variable avg no utilizada
...
async function fetchHourlyData(date: string): Promise<void> {
  ...
  try {
    ...
    const promises = props.selectedLocations.map(async (locationId) => {
      ...
      // Aquí vamos a simular datos horarios basados en la lluvia diaria
      const dailyData = props.historicalData.find(d => d.date === date && d.locationId === locationId);
      
      if (!dailyData) return [];
      
      // Simular datos horarios basados en la precipitación media del día
      // Distribuimos la precipitación total a lo largo de algunas horas del día
      const totalRainfall = dailyData.rainfall || 0;
      const hourlyRainfalls: HourlyData[] = [];
      
      // Si no hay precipitación, devolver datos con lluvia en 0
      if (totalRainfall <= 0) {
        // Crear datos para cada hora del día con lluvia 0
        for (let hour = 0; hour < 24; hour++) {
          const timeString = `${startDate}T${hour.toString().padStart(2, '0')}:00`;
          
          hourlyRainfalls.push({
            time: timeString,
            rainfall: 0,
            locationId,
        ...
        return hourlyRainfalls;
      }
      
      // Si hay precipitación, distribuirla en algunas horas del día
      // Simulamos que llueve durante 6 horas aleatorias a lo largo del día
      const rainyHours: number[] = [];
      const rainfallPerHour = totalRainfall / 6; // Distribuimos en 6 horas
      
      // Generar 6 horas aleatorias (preferentemente en la tarde)
      while (rainyHours.length < 6) {
        // Mayor probabilidad entre las 12 y las 20
        let hour = Math.floor(Math.random() * 24);
        if (Math.random() > 0.7) { // 70% de probabilidad de hora de tarde
          hour = 12 + Math.floor(Math.random() * 8); // Entre 12 y 19
        }
        
        if (!rainyHours.includes(hour)) {
          rainyHours.push(hour);
        }
      }
      
      // Ordenar las horas
      rainyHours.sort((a, b) => a - b);
      
      // Crear datos para cada hora del día
      for (let hour = 0; hour < 24; hour++) {
        const timeString = `${startDate}T${hour.toString().padStart(2, '0')}:00`;
        
        // Calcular la lluvia para esta hora (si es una hora lluviosa)
        const isRainyHour = rainyHours.includes(hour);
        const hourlyRainfall = isRainyHour ? rainfallPerHour : 0;
...
function updateChart() {
  ...
  const chartTitles = {
    days: 'Precipitación Diaria (mm)',
    weeks: 'Precipitación Semanal (mm)',
    months: 'Precipitación Mensual (mm)'
  }
  
  // Crear nuevo gráfico
  chart = new Chart(rainfallChart.value, {
    type: 'bar',
    data: {
      labels,
      datasets
